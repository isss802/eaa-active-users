...
> **⚠️ `unused_candidate` を機械的に削除しないでください。**
>
> このレポートが数えるアクセスイベントには、IdP（ログインポータル）へのログインと、Web アプリ／カスタムドメイン／トンネル型クライアントアクセスへのアクセスが含まれます。`unused_candidate` は、期間内にこれらの記録が無いことを示します。
>
> 名寄せは完全ではありません。無効化の判断前に `needs_review`（弱い一致のみの行）と `active_unmatched`（どのディレクトリにも見つからなかったアクティブ ID）を必ず確認してください。独立した裏取りには公式 CLI の生ログが使えます：`akamai eaa log access -s <epoch> -e <epoch> --json | jq -r 'select(.username != "-") | .username' | sort -u`（要 [cli-eaa](https://github.com/akamai/cli-eaa) と Legacy API キー）。
...
- 記録対象となるアプリケーション種別の網羅は、公式ドキュメントに明記されていません。
